...
namespace DumpMachine {
    ...
    /// }
    /// ```
    function DumpRegister_(register : Qubit[]) : Unit {
        body intrinsic;
    }
    
    @Config(Unrestricted)
    operation CheckZero_(qubit : Qubit) : Bool {
        body intrinsic;
    }
    
    @Config(Unrestricted)
    operation CheckAllZero_(qubits : Qubit[]) : Bool {
        for q in qubits {
            if not CheckZero_(q) {
                return false;
            }
    ...
    function Fact_(actual : Bool, message : String) : Unit {
        if (not actual) {
            fail message;
        }
    }
    ...
    @Config(Unrestricted)
    operation CheckOperationsAreEqual_(
        nQubits : Int,
        actual : (Qubit[] => Unit),
        ...
        let areEqual = CheckAllZero_(reference) and CheckAllZero_(target);
        ResetAll(target);
        ResetAll(reference);
        areEqual
    }
    
    export DumpMachine_, CheckOperationsAreEqual_, DumpRegister_, CheckZero_, CheckAllZero_, Fact_;
}
